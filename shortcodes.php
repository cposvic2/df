...
/**
 * Creates Deedfax Search Tool With Universal Search
 */
function deedfax_search_tool2($atts = []) {
	// Parishes
	$parish_select = '<select class="shipping_method" name="parish" id="parish2">';
	$user_parishes = get_user_parishes();
	if (count($user_parishes)) {
		$default = $user_parishes[0]->getId();
	} else {
	...
	// Universal Search
	$search = '<input class="input-text" type="text" name="search" id="search" value="" placeholder="Optional: enter part or all of an address, subdivision, or street">';
	...
	$text = '
<form id="search-data-form" class="woocommerce-checkout" action="'.get_site_url(null, 'search-results-2').'">
	<div id="customer_details">
		<p class="form-row form-row-wide">
			<label for="parish2" class="">Parish</label>
			'.$parish_select.'
		</p>
		<p class="form-row form-row-wide">
		...
		<input type="hidden" name="action" value="search_data2">
		'.wp_nonce_field( 'search_data', 'search_data_nonce', false ).'
		<input type="submit" value="Search">
	</div>
</form>
...
add_shortcode('deedfax_search_tool2', 'deedfax_search_tool2');