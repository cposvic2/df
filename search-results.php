...
function add_search_results( $content ) {
	if( is_page('search-results') ) {
		...
		if (isset($_REQUEST['search'])) {
			$search = sanitize_text_field($_REQUEST['parish']);
		...
		$response = draw_results($properties, $total_properties, $firstRow, $lastRow, $rowCount, $page, $starting_link);
		/*
		<script>
...
function add_search_results2( $content ) {
	if( is_page('search-results-2') ) {
		$properties = $response = $subdivision_ids = $street_ids = $district_ids = array();
		$parish = $search = $start_date = $end_date = $min_price = $max_price = null;
		...
		if (isset($_REQUEST['search']) && !!$_REQUEST['search']) {
			...
			// Subdivisions first
			$subdivisions = DeedfaxDAOFactory::getDAO('DeedfaxSubdivisionDAO')->searchQuery($search, $parish->getId());
			...
			// Then streets
			$streets = DeedfaxDAOFactory::getDAO('DeedfaxStreetDAO')->searchQuery($search, $parish->getId());
			if (count($streets)) {
		...
		$total_properties = 0;
		if (!!$parish) {
			$properties = DeedfaxDAOFactory::getDAO('DeedfaxPropertyDAO')->searchQuery2($parish->getId(), $subdivision_ids, $street_ids, $district_ids, $min_price, $max_price, $start_date, $end_date, $rowStart, $rowCount);
			$total_properties = DeedfaxDAOFactory::getDAO('DeedfaxPropertyDAO')->searchQueryCount2($parish->getId(), $subdivision_ids, $street_ids, $district_ids, $min_price, $max_price, $start_date, $end_date);	
		}
		...
		$response = draw_results($properties, $total_properties, $firstRow, $lastRow, $rowCount, $page, $starting_link, '/search-3');
		/*
		<script>
			var search_results_markers = '.json_encode($response['markers']).';
		</script>
...
add_filter('the_content', 'add_search_results2');

function draw_results($properties, $total_properties, $firstRow, $lastRow, $rowCount, $page, $starting_link, $new_search_url = '/search') {
	$rowCountOptions = array(5,10,20,50,100);

	$response['markers'] = array();
	$response['html'] ='
	<p><a href="'.get_site_url(null, $new_search_url).'" class="default-btn-shortcode dt-btn dt-btn-m btn-inline-left"><span>Start New Search</span></a></p>
