...
/**
 * Creates Admin subdivision page
 */
function deedfax_admin_subdivisions_callback() {
	callback_start();
	if (isset($_GET['action']) && $_GET['action'] == 'edit' && isset($_GET['id']) && (int)$_GET['id']) {
		display_subdivision_edit();
	} else {
		?><h1>Subdivisions</h1><?php
		$exampleListTable = new Deedfax_Table;
		$exampleListTable->set_type('subdivision');
...
function display_subdivision_edit() {
	$id = (int)$_GET['id'];
	$entry = DeedfaxDAOFactory::getDAO('DeedfaxsubdivisionDAO')->load($id);

	$delete_query = array(
		'id' => $entry->getId(),
		'action' => 'deedfax_subdivision_delete',
		'wpnonce' => wp_create_nonce( 'deedfax_subdivision_delete' )
	);
	...
	?>
		...
		<form method="post" enctype="multipart/form-data" action="<?php echo esc_html(admin_url('admin-post.php')); ?>">
			...
			<input type="hidden" name="action" value="deedfax_subdivision_edit">
			<input type="hidden" name="id" value="<?php echo $entry->getId()?>">
			<?php wp_nonce_field( 'deedfax_subdivision_edit', 'deedfax_subdivision_edit_nonce'); ?>
...
function deedfax_subdivision_edit_callback() {
	...
	if ( wp_verify_nonce($_REQUEST['deedfax_subdivision_edit_nonce'], 'deedfax_subdivision_edit') ) {
		$id = (int)$_REQUEST['id'];
		...
		DeedfaxDAOFactory::getDAO('DeedfaxSubdivisionDAO')->update($entry);
		$saved = 1;
...
add_action('admin_post_deedfax_subdivision_edit', 'deedfax_subdivision_edit_callback');

function deedfax_subdivision_delete_callback() {
	if ( wp_verify_nonce($_REQUEST['wpnonce'], 'deedfax_subdivision_delete') ) {
		$id = (int)$_REQUEST['id'];
		DeedfaxDAOFactory::getDAO('DeedfaxSubdivisionDAO')->delete($id);
		$url = admin_url('admin.php?page=deedfax-subdivisions');
		$deleted = 1;
...
add_action('admin_post_deedfax_subdivision_delete', 'deedfax_subdivision_delete_callback');